...
export default function SystemConfigPage() {
  ...
  const [editKey, setEditKey] = useState(null)       // config_key being edited
  const [editValue, setEditValue] = useState('')
  const [saving, setSaving] = useState(false)
  const [saveMsg, setSaveMsg] = useState(null)        // { key, type: 'success'|'error', msg }
  const [confirmKey, setConfirmKey] = useState(null)  // key awaiting confirmation
  ...
  function startEdit(cfg) {
    ...
    // Show placeholder for masked keys, otherwise show current value
  ...
  function requestSave(cfg) {
    // Show confirmation before committing
    setConfirmKey(cfg.config_key)
  }
  ...
  function renderInput(cfg) {
    if (cfg.data_type === 'boolean') {
      return (
        <select
          ...
          style={{ border: '1px solid #d1d5db', borderRadius: 6, padding: '5px 8px', fontSize: 13, outline: 'none' }}
        >
    ...
    const isSecret = cfg.config_key === 'ANTHROPIC_API_KEY'
    return (
      <input
        type={isSecret ? 'password' : 'text'}
        value={editValue}
        onChange={e => setEditValue(e.target.value)}
        placeholder={cfg.current_value === '***' ? 'Enter new API key…' : cfg.default_value ?? ''}
        style={{ border: '1px solid #d1d5db', borderRadius: 6, padding: '5px 10px', fontSize: 13, outline: 'none', minWidth: 240 }}
  ...
  function formatDisplayValue(cfg) {
    if (cfg.config_key === 'ANTHROPIC_API_KEY') {
      return cfg.current_value === '***' ? <span style={{ color: '#9ca3af' }}>●●●●●●●●</span> : <span style={{ color: '#9ca3af' }}>(not set)</span>
    }
    if (cfg.data_type === 'boolean') {
                    ...
                    <td style={{ padding: '10px 12px' }}>
                      {isEditing ? (
                        <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                          ...
                          <button onClick={cancelEdit} style={{ fontSize: 12, padding: '4px 10px', borderRadius: 6, border: '1px solid #d1d5db', background: '#fff', cursor: 'pointer', color: '#374151' }}>
                            Cancel
                          </button>
                        </div>
                      ) : (
                      ...
                      {msg && (
                        <p style={{ fontSize: 11, marginTop: 4, color: msg.type === 'success' ? '#16a34a' : '#dc2626' }}>{msg.msg}</p>
                      )}
                    </td>
                  </tr>
                )
        ...
        <div style={{ position: 'fixed', inset: 0, zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          ...
          <div style={{ position: 'relative', background: '#fff', borderRadius: 14, padding: '1.5rem', maxWidth: 360, boxShadow: '0 8px 32px rgba(0,0,0,0.15)', zIndex: 10 }}>
            ...
            <p style={{ fontSize: 13, color: '#6b7280', marginBottom: 20 }}>
              Update <strong style={{ fontFamily: 'monospace' }}>{confirmKey}</strong> to the new value?
              This will take effect immediately.
            </p>
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
              <button onClick={() => setConfirmKey(null)} style={{ fontSize: 13, padding: '7px 16px', borderRadius: 8, border: '1px solid #d1d5db', background: '#fff', cursor: 'pointer', color: '#374151' }}>Cancel</button>
              <button onClick={confirmSave} style={{ fontSize: 13, padding: '7px 16px', borderRadius: 8, background: '#1A6B7B', color: '#fff', border: 'none', cursor: 'pointer', fontWeight: 600 }}>Confirm Save</button>
            </div>
          </div>
        </div>
      )}
